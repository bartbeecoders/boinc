...
Remote: https://github.com/bartbeecoders/boinc (CI green on all platforms incl. arm64 Linux; v0.2.0 released with deb/rpm — x86_64 + aarch64 — msi/dmg artifacts). Local clippy (Rust 1.95) misses newer-toolchain lints that CI's stable catches — e.g. ambiguous float literals for `impl Into<f32>` params need explicit `_f32` suffixes.
