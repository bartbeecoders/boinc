...
Remote: https://github.com/bartbeecoders/boinc (CI green on all three platforms; v0.1.0 released with deb/rpm/msi/dmg artifacts). Local clippy (Rust 1.95) misses newer-toolchain lints that CI's stable catches — e.g. ambiguous float literals for `impl Into<f32>` params need explicit `_f32` suffixes.
